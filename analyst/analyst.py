...
def _live_process_names() -> set:
    ...
    live/gone tag that keeps 24h-cumulative panels honest (a dead process
    like the transient 'MainThread' no longer reads as active)."""
    names = set()
    try:
...
def _demo_stats() -> dict:
    ...
    return {
        ...
        "top_processes": [  # roughly reconciled with by_company above
            {"process": "chrome.exe", "bytes": 769_000_000, "live": True},   # Google+Netflix+Meta via browser
            {"process": "primevideo.exe", "bytes": 524_000_000, "live": True},
            {"process": "svchost.exe", "bytes": 187_000_000, "live": True},  # delivery optimization
            {"process": "spotify.exe", "bytes": 143_000_000, "live": True},
            {"process": "onedrive.exe", "bytes": 21_000_000, "live": True},
        ],
...
function renderProcesses(list){
  ...
  el.innerHTML = entries.map(p =>
    ...
    + (p.live===true ? '<span class="lv on" title="running right now">● live</span>'
       : (p.live===false ? '<span class="lv off" title="no longer running — 24h history">○ gone</span>' : ''))
