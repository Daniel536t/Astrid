...
def _stats_totals() -> dict:
    ...
    b = _ch_rows(f"""
        SELECT sum(d) AS bytes FROM (
          SELECT fingerprint, metric_name, max(value) - min(value) AS d
          FROM signoz_metrics.samples_v4
          WHERE metric_name IN ('net.bytes_sent','net.bytes_recv') AND unix_milli > {cutoff24}
...
@app.get("/api/stats")
def api_stats():
    """Live visualization bundle for Console v2. Cached ~2s."""
    now = time.time()
